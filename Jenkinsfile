...
node("nodejs"){
    ...
    stage('Copy files'){
        ...
        if (success) {
            catchError(buildResult: 'FAILURE', stageResult: 'FAILURE') {
                ...
                sh "cp /root/keys/kapusta-backend/.env /root/slave_jenkins/workspace/Build-Bookread-Backend/.env"
            }
        }
    }
    ...
    stage('Deploy') {
        ...
        if (success) {
            catchError(buildResult: 'FAILURE', stageResult: 'FAILURE') {
                //sent files to  bookread-backend.goit.global
                sh "scp -r ./* ${env.sshUserAndHost}:/root/bookread-backend"
            }
        }
    }
